...
{
    public class SerializerCSV : Serializer
    {
        public List<PropertyInfo> _properties;
        public List<FieldInfo> _fields;

        public SerializerCSV(Type type)
        {
            _properties = type.GetProperties(bindingAttr: BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public).ToList();
            _fields = type.GetFields(bindingAttr: BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public).ToList();
        }
        ...
        private Tuple<string, string> GetKeyValuePairInCsvFormat(ISerializable entity, string separator)
        {
            ...
            StreamingContext streamingContext = new(StreamingContextStates.Persistence);
